...
import { airdropAll, getConfig, buildAndSendTx, getTokenInfo, getTournament, Role, getProgram, getProvider, getSponsorAtaInfo, getPrizePoolAtaInfo, getOrganizerAtaInfo, getGenomePda, getConstant } from "../common/utils";
...
describe("Genome Solana Singlechain", () => {
    ...
    it("Initialize Genome Solana with Invalid Params", async () => {
        try {
            ...
            const initIx = await ixBuilder.initializeSingleIx(deployer.publicKey, configDataInvalid);
    ...
    it("Initialize Genome Solana", async () => {
        const initIx = await ixBuilder.initializeSingleIx(deployer.publicKey, configData);
