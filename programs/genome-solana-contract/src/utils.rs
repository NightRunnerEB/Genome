...
pub fn validate_params(params: &TournamentConfig, config: &GenomeConfig, token_info: &TokenInfo) -> Result<()> {
    let clock = Clock::get()?;
...
pub fn initialize_bloom_filter(tournament_config: &TournamentConfig, false_precision: &f64, bloom_filter: &mut Account<BloomFilter>) -> Result<()> {
