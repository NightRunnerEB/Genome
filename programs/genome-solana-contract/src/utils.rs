...
pub fn calculate_bloom_memory(players_count: u16, false_precision: u64) -> Result<usize> {
    if players_count > MAX_TEAM_SIZE {
        return Err(TournamentError::MaxPlayersExceeded.into());
    }
    let false_precision = (false_precision as f64) / 1_000_000_000.0;
...
pub fn validate_params(params: &TournamentData, config: &GenomeConfig) -> Result<()> {
    require!(
        ...
        TournamentError::InvalidOrganizerFee
    );
    require!(
